...
export const openRazorpayModal = async (options) => {
  ...
  const defaultOptions = {
    theme: {
      color: "#161616",      // Primary dark color
      backdrop_color: "#ffe343", // Yellow accent color (for background)
      hide_topbar: false,
    },
    modal: {
      backdropclose: false,
...
export const createCreditOrder = async (credits, token) => {
  const response = await fetch(`${BACKEND_URL}/payment/create-credit-order`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const verifyCreditPayment = async (orderId, paymentId, signature, token) => {
  const response = await fetch(`${BACKEND_URL}/payment/verify-credit-payment`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
